{
	...
	"lastUpdated": "2013-02-07 23:19:43"
}
...
FW.Scraper({ itemType         : 'newspaperArticle',
			 detect           : FW.Xpath('//div[@id="hypercontext"]/h1'),
			 title            : FW.Xpath('//div[@id="hypercontext"]/h1').text().trim(),
			 attachments      : [
			 {
			   url : FW.Url(),
			   type : "text/html",
			 ...
			 creators         : FW.Xpath('//div[@class="fr"]/p').text().replace(/\n.+[\n\t]*.+/, "").trim().cleanAuthor("author"),
			 date 	      : FW.Xpath('//div[@id="hypercontext"]/div[@class="top"]').text().match(/(\d{1,2}\.\d{1,2}\.\d{4})/,1),
