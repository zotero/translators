{
	...
	"lastUpdated": "2013-02-22 00:05:36"
}
...
FW.Scraper({
itemType : 'report',
detect : FW.Xpath('//div[@class="paperdetail-h1"]//h1'),
title : FW.Xpath('//div[@class="paperdetail-h1"]//h1').text().trim(),
attachments : {
  url : FW.Url(),
  title : "SAE Technical Papers Snapshot",
  type : "text/html"
},
creators : FW.Xpath('//div[@class="prod-detail-authors"]').text().split(/\n/).remove(/-.+/).cleanAuthor("author"),
abstractNote : FW.Xpath('//div[@class="pap-detail-abstract"]').text().trim().remove(/\n.+/g),
number:	FW.Xpath('//div[@class="prod-details-prodcode"]/span').text().trim(),	
date : FW.Xpath('//div[@class="prod-details-published"]').text().remove(/\s*Date Published:\s*/),
DOI  : FW.Xpath('//div[@class="prod-detail-doi"]').text().remove(/\s*DOI:\s*/),
...
FW.MultiScraper({
itemType : "multiple",
detect : FW.Xpath('//div[@class="brw-body-title"]/a'),
choices : {
  titles : FW.Xpath('//div[@class="brw-body-title"]/a').text(),
  urls : FW.Xpath('//div[@class="brw-body-title"]/a').key('href').text()
}
});
/** BEGIN TEST CASES **/
var testCases = [
