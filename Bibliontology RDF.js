{
	...
	"lastUpdated": "2019-05-07 22:00:28"
}
...
var TYPES = {
	...
	newspaperArticle:	[[[n.rdf + "type", n.bibo + "Article"]], 				[true,
		n.dcterms + "isPartOf",
...
var SAME_ITEM_RELATIONS = [n.dcterms + "isPartOf",
	n.dcterms + "isVersionOf",
	n.bibo + "affirmedBy",
	n.bibo + "presentedAt",
	n.bibo + "presents",
	n.bibo + "reproducedIn",
	n.bibo + "reviewOf",
	n.bibo + "translationOf",
	n.bibo + "transcriptOf"];
...
function getBlankNode(attachToNode, itemPredicate, blankNodePairs, create) {
	...
	if (!blankNode && create) {
		...
		for (let i = 0; i < blankNodePairs.length; i++) {
 ...
 */
const Type = function (type, typeDefinition) {
	this.zoteroType = type;
	this[ITEM] = { pairs: typeDefinition[0] };
	this[SUBCONTAINER] = typeDefinition[1]
		? { alwaysAdd: typeDefinition[1][0], predicate: typeDefinition[1][1], pairs: typeDefinition[1][2] }
		: null;
	this[CONTAINER] = typeDefinition[2]
		? { alwaysAdd: typeDefinition[2][0], predicate: typeDefinition[2][1], pairs: typeDefinition[2][2] }
		: null;
};
...
Type.prototype._scoreNodeRelationship = function (node, definition, score) {
	var subNode = null;
	if (definition) {
		const statements = Zotero.RDF.getStatementsMatching(node, definition.predicate, null);
		if (statements) {
			var bestScore = -9999;
			for (var i = 0; i < statements.length; i++) {
...
Type.prototype.addNodeRelations = function (nodes) {
	// add node relations
	let list = [ITEM_SERIES, SUBCONTAINER_SERIES, CONTAINER_SERIES];
	...
	list = [ITEM, SUBCONTAINER, CONTAINER];
	let predicate;
	for (let h = 0; h < list.length; h++) {
		const i = list[h];
		if (nodes[i]) {
			...
			if (i == ITEM) {
				// what does this do?
				const j = 1; // eslint-disable-line no-unused-vars
				predicate = n.res + "resource";
			}
				...
				predicate = this[i].predicate;
			}
			
			// add type
			for (let j = 0; j < this[i].pairs.length; j++) {
 ...
 */
const LiteralProperty = function (field) {
	this.field = field;
...
LiteralProperty.prototype.mapToItem = function (newItem, nodes) {
	if (typeof this.mapping[0] == "function") {		// function case: triples returned
		// check function case
		const content = this.mapping[1](nodes);
 ...
 */
const CreatorProperty = function (field) {
	this.field = field;
...
CreatorProperty.prototype.mapToCreator = function (creatorNode, zoteroType) {
	...
	if (this.field == "author") {
		...
		if (!creatorsForType.includes("author")) {
...
CreatorProperty.prototype.mapFromCreator = function (item, creator, nodes) {
	...
	// attach creator node
	let attachTo = nodes[mapping[0]];
	let relation;
	if (typeof mapping[2] == "string") {
		relation = mapping[2];
	}
	else {
		relation = mapping[2][2];
		attachTo = getBlankNode(attachTo, mapping[2][0], mapping[2][1], true);
	}
	...
	// add to creator list
	let creatorList = Zotero.RDF.getStatementsMatching(nodes[mapping[0]], CREATOR_LISTS[list], null);
	if (creatorList) {
		creatorList = creatorList[0][2];
	}
	else {
		creatorList = Zotero.RDF.newResource();
...
function doImport() {
	...
	for (var unprefixedBiboType in BIBO_TYPES) {
		...
		const type = new Type(BIBO_TYPES[unprefixedBiboType], [[[RDF_TYPE, n.bibo + biboType]], null, null]);
	...
	for (let zoteroType in TYPES) {
		const type = new Type(zoteroType, TYPES[zoteroType]);
		for (let i = 0; i < TYPES[zoteroType][0].length; i++) {
	...
	for (var zoteroField in FIELDS) {
		if (typeof FIELDS[zoteroField][0] == "function") {
			...
			const domain = FIELDS[zoteroField][0];
			let predicate = FIELDS[zoteroField][1];
			if (typeof predicate == "object") predicate = predicate[0];
			const prop = new LiteralProperty(zoteroField);
	...
	for (var creatorType in CREATORS) {
		const domain = CREATORS[creatorType][0];
		let predicate = CREATORS[creatorType][2];
		if (typeof predicate == "object") predicate = predicate[0];
		const prop = new CreatorProperty(creatorType);
	...
	for (let h in itemNodes) {
		itemNode = itemNodes[h];
		...
		for (var j = 0; j < arcs.length; j++) {
			if (SAME_ITEM_RELATIONS.include(arcs[j])) {
				skip = true;
				break;
			}
		}
		...
		for (let j = 0; j < itemRDFTypes.length; j++) {
			const rdfType = itemRDFTypes[j];
			...
			for (var k = 0; k < collapsedTypesForItem.length; k++) {
				const type = collapsedTypesForItem[k];
		...
		// create item
		const zoteroType = bestType.zoteroType;
		...
		for (let i in nodes) {
			...
			for (var g = 0; g < properties.length; g++) {
				...
				if (propertyMappings) {
					for (let k = 0; k < propertyMappings.length; k++) {
		...
		for (let j in functionProperties) {
		...
		for (let i in nodes) {
			...
			let statements = Zotero.RDF.getStatementsMatching(nodes[i], n.dcterms + "subject", null);
			...
			statements = Zotero.RDF.getStatementsMatching(nodes[i], n.ctag + "tagged", null);
			for (let j = 0; j < statements.length; j++) {
				...
				for (let k = 0; k < types.length; k++) {
					const type = types[k];
					const uri = Zotero.RDF.getResourceURI(type[2]);
					if ([n.ctag + "AutoTag", n.ctag + "AuthorTag"].includes(uri)) tag.type = 1;
					break;
				}
			...
			for (let j in CREATOR_LISTS) {
				statements = Zotero.RDF.getStatementsMatching(nodes[i], CREATOR_LISTS[j], null);
				for (let k = 0; k < statements.length; k++) {
					...
					const creatorNodes = Zotero.RDF.getContainerElements(stmt[2]);
					for (var l = 0; l < creatorNodes.length; l++) {
						var creatorNode = creatorNodes[l];
						const creatorNodeURI = Zotero.RDF.getResourceURI(creatorNode);
						if (!creatorsAdded[creatorNodeURI]) {
							...
							if (allCreators[creatorNodeURI]) {
								...
								if (j == AUTHOR_LIST) {
									...
									prop = new CreatorProperty("author");
								}
		...
		for (const creatorNodeURI in allCreators) {
			if (!creatorsAdded[creatorNodeURI]) {
				newItem.creators.push(allCreators[creatorNodeURI]);
			}
		}
...
function doExport() {
	...
	for (var h in items) {
		...
		for (var field in item.uniqueFields) {
			...
			const property = new LiteralProperty(field);
		...
		for (let i = 0; i < item.creators.length; i++) {
			...
			// create creator
			const property = new CreatorProperty(creator.creatorType);
		...
		for (let i = 0; i < item.tags.length; i++) {
			...
			if (tagCollection[tag.tag]) {
				tagNode = tagCollection[tag.tag];
			}
			else {
				tagNode = Zotero.RDF.newResource();
