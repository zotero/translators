...
function processTag(item, entry) {
	...
	switch(tag) {
		...
		case "PY":
			item.backupDate = [zField, dateRIStoZotero(value)];
			value = undefined;
			processFields = false;
		break;
	}
...
function dateRIStoZotero(risDate) {
	var value = risDate.split('/');

	//adjust month (it's 0 based)
	if(value[1]) {
		value[1] = parseInt(value[1].replace(/^0+(?=\d)/,'')); //drop leading zeroes
		if(value[1]) value[1]--;
	}
...
function completeItem(item) {
	...
	if(item.backupDate) {
		if(!item[item.backupDate[0]]) {
			item[item.backupDate[0]] = item.backupDate[1];
		}
