{
	...
	"lastUpdated": "2014-05-03 15:06:36"
}
...
attachments      : [{ url: FW.Url(),  //.replace(/www\.csmonitor\.com/, "www.csmonitor.com/layout/set/print") once print issue is resolved
...
detect           : FW.Xpath('//div[@id="mainColumn"]/h1[@class="head"]|//div[@id="mainColumn"]//h1[@itemprop="name"]'),
title            : FW.Xpath('//h1[@class="head"]|//h1[@itemprop="name"]').text().trim(),
attachments      : [{ url: FW.Url(),
  title:  "CS Monitor Snapshot",
  type: "text/html" }],
creators         : FW.Xpath('//p[@class="sByline"]').text().capitalizeTitle("true").replace(/By\s*/, "").replace(/,.*/, "").capitalizeTitle("true").cleanAuthor("author"),
date             : FW.Xpath('//p[@class="sByline"]').text().replace(/[\n\t]+/g, "").replace(/.+\//, "").replace(/By/, ""),
abstractNote     : FW.Xpath('//meta[@name="description"]/@content').text(),
ISSN			 : "0882-7729",
