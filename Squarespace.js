...
function getSearchResults(doc, checkOnly) {
	...
	for (let row of rows) {
		let href = row.parentElement.attributes['data-url']?.value;
...
function getMetadataItem(doc, prop) {
	return attr(doc, 'meta[itemprop="' + prop + '"]', 'content');
}
...
async function scrape(doc, url = doc.location.href) {
	...
	translator.setHandler('itemDone', (_obj, item) => {
		...
		if (author) item.creators.push(ZU.cleanAuthor(author, 'author'));
		if (item.abstractNote) item.abstractNote = ZU.trimInternal(ZU.unescapeHTML(item.abstractNote));
		item.tags = Array.from(doc.querySelectorAll('.blog-item-tag-wrapper a')).map((tag) => ZU.trim(tag.textContent));
		item.complete();
	});
