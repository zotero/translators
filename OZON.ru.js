{
	...
	"lastUpdated": "2012-06-30 11:04:47"
}
...
function detectWeb(doc, url) {
	...
	var itemTypeDOMNode = doc.evaluate(itemTypeXPath, doc, nsResolver, XPathResult.ANY_TYPE, null).iterateNext();
	var itemType;
	...
	if (itemType == "Книги") {
		...
		itemTypeDOMNode = doc.evaluate(itemTypeXPath, doc, nsResolver, XPathResult.ANY_TYPE, null).iterateNext();
		if (itemTypeDOMNode) itemType = itemTypeDOMNode.textContent
		else itemType = "";
...
function IsAudioBook(nameXPathRes, valueXPathRes) {
	var nameDOMNode;
	var valueDOMNode;
	while ((nameDOMNode = nameXPathRes.iterateNext()) && (valueDOMNode = valueXPathRes.iterateNext())) {
...
function ParseProductDetail(pdXPathRes) {
	...
	if (creatorsObject["authors"]) {
		creatorsObject["all"] = creatorsObject["all"].concat(creatorsObject["authors"]);
	}
	if (creatorsObject["compilers"]) {
		creatorsObject["all"] = creatorsObject["all"].concat(creatorsObject["compilers"]);
	}
	if (creatorsObject["translators"]) {
		creatorsObject["all"] = creatorsObject["all"].concat(creatorsObject["translators"]);
	}
	if (creatorsObject["editors"]) {
		creatorsObject["all"] = creatorsObject["all"].concat(creatorsObject["editors"]);
	}
...
function ParseDetailDesc(nameXPathRes, valueXPathRes) {
	...
	while ((nameDOMNode = nameXPathRes.iterateNext()) && (valueDOMNode = valueXPathRes.iterateNext())) {
		...
		case "От производителя":
			valueText = valueDOMNode;
			result["abstractNote"] = valueText.textContent.replace(/От производителя[\s\n\t]*/, "");
			break;
		case "От OZON.ru":
			valueText = CleanText(valueDOMNode.textContent);
			if (valueText.match("издание\.$")) result["edition"] = GetEdition(valueText).replace(/От OZON.ru[\s\n\t]*/, "");
			break;
		}
	}
	return result;
