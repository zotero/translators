{
	...
	"lastUpdated": "2017-03-16 02:45:10"
}
...
title            : FW.Xpath('//div[@id="article_view"]/h1').text(), // og:title has inaccurate info
attachments      : [{ url: FW.Url(),
  title:  "Yale Alumni Magazine snapshot",
  type: "text/html" }],
creators         : FW.Xpath('//span[@class="author_names"]/a').text().cleanAuthor("author"),
date             : FW.Xpath('//a[@class="text_link"]').text(),
publicationTitle : "Yale Alumni Magazine",
language         : "en-US",
ISSN             : "0044-0051",
volume           : FW.Xpath('//div[@class="current_issue_container"]/div').text().replace(/Vol\s([A-z]+).*/,'$1'),
issue            : FW.Xpath('//div[@class="current_issue_container"]/div').text().replace(/Vol\s[A-z]+,\sNo\s([0-9]+)/,'$1'),
abstractNote     : FW.Xpath('//meta[@name="description"]/@content').text(),
tags             : FW.Xpath('//ul[@id="filed_under"]/li/a').text().trim()
});

FW.Scraper({
itemType         : 'blogPost',
detect           : FW.Url().match(/\/blog_posts\//),
title            : FW.Xpath('//h1[@id="blog_post_title"]').text(),
attachments      : [{ url: FW.Url(),
  title:  "Yale Alumni Magazine blog post snapshot",
  type: "text/html" }],
creators         : FW.Xpath('//span[@class="blog_post_author_unlinked"]').text().replace(/\s’\d{2}$/,"").cleanAuthor("author"),
date             : FW.Xpath('//div[@class="blog_post_authors"]').text().replace(/([\w\s\\n\s\|’]*)(\d{1,2}:\d{2}\w{2})\s(\w+\s\d+)\s(\d+)/,"$3, $4, $2 "),
...
FW.MultiScraper({
itemType         : 'multiple',
detect           : FW.Url().match(/(\/issues\/\d+$)|(\/magazine\/(departments|topics)\/)/), // issue TOC & department/topical pages
choices          : {
  titles  :  FW.Xpath('//a[@class="article_linked_headline "]').text().trim(),
  urls    :  FW.Xpath('//a[@class="article_linked_headline "]').key("href").trim()
}
});

FW.MultiScraper({
itemType         : 'multiple',
detect           : FW.Url().match(/\/(search\?|blogs\/)/), // search and blog topic pages
choices          : {
  titles  :  FW.Xpath('//h2/a').text().trim(),
  urls    :  FW.Xpath('//h2/a').key("href").trim()
}
});
/** BEGIN TEST CASES **/
var testCases = [
