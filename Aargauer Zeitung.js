	...
	if (tickerEntries.length > 0) {
		...
		for (var i = 0; i < tickerArray.length; i++) {
			...
			for (var p of paragraphs) {
				if (paywallSet.has(p)) continue;
				var txt = getFormattedText(p);
				if (txt) {
		...
		for (var el of articleElements) {
			...
			if (headingMap[tag]) {
				var headingText = ZU.trimInternal(el.textContent);
				if (headingText) {
					var newTag = headingMap[tag];
					fullTextHtml.push('<' + newTag + '>' + headingText + '</' + newTag + '>');
				}
			}
			else {
				var txt = getFormattedText(el);
				if (txt) {
