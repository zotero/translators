{
    ...
    "lastUpdated": "2014-07-03 21:36:42"
}
...
function doWeb(doc, url) {
        ...
        if (alsoAvailable) {
            item.numPages = ZU.trimInternal(pages2.split(' ')[0]);
            item.date = ZU.trimInternal(date_isbn_2.split('|')[2]);
            item.ISBN = ZU.trimInternal(date_isbn_2.split('|')[0]);
        }
        else {
            item.numPages = ZU.trimInternal(pages1.split(' ')[0]);
            item.date = ZU.trimInternal(date_isbn_1.split('|')[2]);
            item.ISBN = ZU.trimInternal(date_isbn_1.split('|')[0]);
        }
