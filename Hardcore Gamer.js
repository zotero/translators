{
	...
	"lastUpdated": "2017-03-12 06:46:07"
}
...
attachments      : [{ url: FW.Url(),
  title:  "Hardcore Gamer snapshot",
  type: "text/html" }],
creators         : FW.Xpath('//a[@rel="author"]').text().replace(/\son\s.*/,'').cleanAuthor("author"),
date             : FW.Xpath('//a[@rel="author"]').text().replace(/.*\son\s/,''),
...
FW.MultiScraper({
itemType         : 'multiple',
detect           : FW.Url().match(/(\/games\/)|(\/\?s=)/), // game pages or search (the latter contained simultaneous classes, hence the longer XPath)
choices          : {
  titles  :  FW.Xpath('//h1[contains(concat(" ", normalize-space(@class), " "), " entry-title ")]/a').text().trim(),
  urls    :  FW.Xpath('//h1[contains(concat(" ", normalize-space(@class), " "), " entry-title ")]/a').key("href").trim()
  }
});/** BEGIN TEST CASES **/
var testCases = [
	{
		"type": "web",
		"url": "http://www.hardcoregamer.com/2016/02/02/pax-south-want-to-ruin-a-friendship-play-videoball/190827/",
		"items": [
			{
				"itemType": "blogPost",
	...
	{
		"type": "web",
		"url": "http://www.hardcoregamer.com/games/overwatch/",
		"items": "multiple"
	},
	{
		"type": "web",
		"url": "http://www.hardcoregamer.com/?s=earthbound",
		"items": "multiple"
	}
]
/** END TEST CASES **/