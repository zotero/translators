{
	...
	"lastUpdated": "2015-08-09 05:27:45"
}
...
function getAuthorFromByline(doc, newItem) {
	...
	if(actualByline) {
		//Z.debug(actualByline.innerHTML)
		byline = ZU.trimInternal(actualByline.textContent);
		var authors = [];
		//make sure we're separating multiple authors on same line in byline
		if (ZU.xpath(actualByline, './/*[string-length(text())>1]').length > 2){
			//but only grab them once
			var bylineNodes = ZU.xpath(actualByline, './/*[string-length(text())>1]/text()');
			Z.debug("Trying " + bylineNodes.length + " byline nodes")
			for (var i = 0; i<bylineNodes.length; i++){
				if (bylineNodes[i].textContent && bylineNodes[i].textContent.trim()){
					authors.push(bylineNodes[i].textContent.trim())				
				}
			}
		}
		else{
			...
			authors = byline.split(/\s*(?:(?:,\s*)?and|,|&)\s*/i);				
		}
