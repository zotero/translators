{
	...
	"lastUpdated": "2020-01-28 05:21:56"
}
...
function addHighwireMetadata(doc, newItem) {
	...
	newItem.creators = [];
	for (let i = 0, n = authorNodes.length; i < n; i++) {
		var authors = authorNodes[i].nodeValue.split(/\s*;\s*/);
		if (authors.length == 1 && authorNodes.length == 1) {
			// If there is only one author node and we get nothing when splitting by semicolon,
			// and at least two words on either side of the comma when splitting by comma,
			// we split by comma
		...
		for (let j = 0, m = authors.length; j < m; j++) {
	...
	if (!newItem.creators.length) {
		...
		for (let i = 0; i < newItem.creators.length; i++) {
			let name = newItem.creators[i].firstName
				+ newItem.creators[i].lastName;
			for (let j = 0; j < rdfCreators.length; j++) {
				let creator = rdfCreators[j];
...
function addLowQualityMetadata(doc, newItem) {
	...
	if (!newItem.creators.length) {
		...
		if (w3authors.length > 0) {
			for (let i = 0; i < w3authors.length; i++) {
				// skip empty authors. Try to match something other than punctuation
...
/**
 * Returns an array of objects of Og authors, but only where they do not contain a URL to prevent
 * getting Facebook profiles. In a worst case scenario, where real authors and social media profiles
 * are mixed, we might miss some, but that's still preferable to garbage.
 */
function tryOgAuthors(doc) {
	...
	for (let i = 0; i < ogAuthors.length; i++) {
...
function getAuthorFromByline(doc, newItem) {
	var bylineClasses = ['byline', 'bylines', 'vcard'];
	Z.debug("Looking for authors in " + bylineClasses.join(', '));
	var bylines = [], byline;
	for (let i = 0; i < bylineClasses.length; i++) {
	...
	if (actualByline) {
		let byline = ZU.trimInternal(actualByline.textContent);
		...
		if (li.length) {
			...
			let authors = byline.split(/\s*(?:(?:,\s*)?and|,|&)\s*/i);
			if (authors.length == 2 && authors[0].split(' ').length == 1) {
				...
				for (let author of authors) {
					if (!author.length || author.includes('@')) {
					...
					if (author.split(/\s/).length == 1) {
						...
						newItem.creators.push(
							ZU.cleanAuthor(fixCase(author), 'author'));
					}
				}
			}
		}
...
/**
 * If we already have tags, run through them one by one, split where necessary, and concat them.
 *
		...
		if (exports.splitTags) {
			...
			for (let tag of newItem.tags) {
				...
				if (!tag.includes(';')) {
					...
					tags = tags.concat(tag.split(/\s*,\s*/));
				}
				else {
					tags = tags.concat(tag.split(/\s*;\s*/));
				}
