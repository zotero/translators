{
	...
	"translatorType": 4,
	"browserSupport": "g",
	"lastUpdated": "2012-05-15 05:40:36"
}
...
FW.Scraper({
	...
	url: FW.Url().split("#").split("?"),
	title: FW.Xpath('//head/title').text().match(": (.*)",1),
	creators: function(){ return [{ "lastName":"DrugBank", "creatorType":"editor", "fieldMode":1 }]  },
	attachments: [{
		url: FW.Url().split("#").split("?").append(".mol"),
		title: "MOL file download",
		type: "text/plain"
	}, {
		url: FW.Url().split("#").split("?").append(".sdf"),
		title: "SDF Molecule file",
		type: "text/plain"
	}, {
		url: FW.Url().split("#").split("?").append(".smiles"),
		title: "SMILE file",
		type: "text/plain"
	}],
	abstractNote: FW.Xpath('//tr/td[preceding-sibling::*="Description"]/p').text(),
	date: FW.Xpath('//div/div[@id="drug-meta"]').text().match(/Drug created on ([a-zA-Z0-9 ,:]+) \/ Updated on ([a-zA-Z]+ [0-9]+, [0-9]{4,4}) /, 2),
//	tags: FW.Xpath('//div[@class="terms terms-inline"]/ul/li/a[@rel="tag"]').text(),
	encyclopediaTitle: "DrugBank"
});

//May 04, 2012 12:22/** BEGIN TEST CASES **/
var testCases = [
	{
		"type": "web",
