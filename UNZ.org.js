{
	...
	"lastUpdated": "2017-03-15 05:15:44"
}
...
detect           : FW.Url().match(/\/Pub\/.*-\d{5}/), // only detect individual article pages – http://regexr.com/3fh53
title            : FW.Xpath('//div[@class="Head"]/a[@class="ArticleLink"]').text(),
attachments      : [{ 
	url: FW.Url().append("?View=Overview"), // capture the Overview page, not the PDF preview default
	title:  "UNZ.org overview snapshot",
	type: "text/html" 
  },{ 
	url: FW.Xpath('//div[@class="inblock"]/iframe/@src').text(), // capture the PDF article as well
	title:  FW.Xpath('//div[@class="Head"]/a[@class="ArticleLink"]').text(),
	type: "application/pdf"
  }],
creators         : FW.Xpath('//div[@class="Byline"]/a').text().cleanAuthor("author"),
date             : FW.Xpath('//div[@class="SourceLine"]/a').text(),
pages            : FW.Xpath('//div[@class="SourceLine"]').text().replace(/.*p+\.\s(\d+(-\d+)?).*/,'$1'), // http://regexr.com/3du86
publicationTitle : FW.Xpath('//div[@class="SourcePub"]').text(),
abstractNote     : FW.Xpath('//meta[@name="description"]/@content').text(),
hooks : { "scraperDone": function  (item,doc,url) {
	// for book reviews: pull reviewedAuthor and tag with book title
	var subheadExists = Zotero.Utilities.xpath(doc, '//div[@class="PubText"]//div[@class="Subhead"]')
	if (Object.keys(subheadExists).length == 1) { // test whether previous XPath was positive
		subheadExists = subheadExists[0]["innerText"];
		var reviewedAuthor = Zotero.Utilities.xpath(doc, '//div[@class="Subhead"]')[0]["innerText"].replace(/.*\sby\s(.*)/,'$1');
		...
		item.tags[0] = Zotero.Utilities.xpath(doc, '//div[@class="Subhead"]')[0]["innerText"].replace(/(.*),\sby\s.*/,'$1');
	}
}}
});
...
FW.MultiScraper({
itemType         : 'multiple',
detect           : FW.Xpath('//div[@class="Draggable box" or @class="PubText box"]'), // works on all author, magazine, magazine TOC, generic article search pages – see http://regexr.com/3fh64 for hours of now-unused regex
choices          : {
  titles  :  FW.Xpath('//a[@class="ArticleLink"]').text().trim(), // I originally included books (@class="BookLink") in this XPath too, but now exclude them as they have no useful metadata
  urls    :  FW.Xpath('//a[@class="ArticleLink"]').key("href").trim()
}
});
/** BEGIN TEST CASES **/
var testCases = [
	{
		"type": "web",
		"url": "http://www.unz.org/Pub/NYRevBooks-1969nov20-00027",
		"items": [
			{
				"itemType": "magazineArticle",
	...
	{
		"type": "web",
		"url": "http://www.unz.org/Pub/Freeman-1958oct-00058",
		"items": [
			{
				"itemType": "magazineArticle",
	...
	{
		"type": "web",
		"url": "http://www.unz.org/Pub/NewRepublic-1943dec20-00878",
		"items": [
			{
				"itemType": "magazineArticle",
	...
	{
		"type": "web",
		"url": "http://www.unz.org/Author/GoodmanPaul",
		"items": "multiple"
	},
	{
		"type": "web",
		"url": "http://www.unz.org/Pub/Politics-1946nov",
		"items": "multiple"
	},
	{
		"type": "web",
