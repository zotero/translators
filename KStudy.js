{
	...
	"translatorType": 4,
	"browserSupport": "gcsi",
	"lastUpdated": "2017-05-25 22:14:50"
}
...
FW.Scraper({
	...
	detect		:	FW.Url().match(/(\/journal\/thesis_name\.asp)/),
	title		:	FW.Xpath('//div[@class="agency_box"]/div[1]/span').text(),
	language	:	"kr",
	creators	:	FW.Xpath('//div[@class="agency_detail"]/ul/li[1]').text().split(/,/).trimInternal().cleanAuthor("author", false),
	date		:	FW.Xpath('//div[@class="agency_detail"]/ul/li[2]').text().match(/(\d\d\d\d)/),
	publicationTitle	:	FW.Xpath('//div[@class="agency_detail"]/ul/li[2]').text().match(/(<[\D]+>)/).remove(/</).remove(/>/),
	pages		:	FW.Xpath('//div[@class="agency_detail"]/ul/li[2]').text().match(/(p+\.\d+-\d+)/).remove(/p+\./),
	volume		:	FW.Xpath('//div[@class="agency_detail"]/ul/li[2]').text().match(/(>\s+\d+)/).remove(/>\s+/).remove(/0/),
	issue		:	FW.Xpath('//div[@class="agency_detail"]/ul/li[2]').text().match(/(>\s+\d+\D\d+)/).remove(/>\s+\d+\D/).remove(/0/),
	abstractNote:	FW.Xpath('//div[@class="con_detail"]/div[4]/ul').text().trimInternal(),
});

/** Search Results */
FW.MultiScraper({
	itemType	:	'multiple',
	detect		:	FW.Url().match(/(\/journal\/list_name\.asp|\/search\/result_kiss\.asp)/),
	choices		:	{
		titles	:	FW.Xpath('//li[@class="title"]/a').text().trim(),
		urls	:	FW.Xpath('//li[@class="title"]/a').key('href').text()
	}
});/** BEGIN TEST CASES **/
var testCases = [
	{
