{
	...
	"lastUpdated": "2017-03-15 06:42:59"
}
...
FW.Scraper({
itemType         : 'blogPost',
detect           : FW.Url().match(/\/article\//), // news, reviews, guides, and features
title            : FW.Xpath('//meta[@property="og:title"]/@content').text(),
attachments      : [{ url: FW.Url(),
  title:  "Shacknews snapshot",
  type: "text/html" }],
creators         : FW.Xpath('//div[@class="author"]').text().replace(/Written\sBy\s/,"").cleanAuthor("author"),
date             : FW.Xpath('//div[@class="date clear-fix"]').text().replace(/[\s\n]+/g," ").replace(/(\d+)\s(\w+)/,"$2 $1,"),
blogTitle        : "Shacknews",
language         : "en-US",
...
FW.MultiScraper({
itemType         : 'multiple',
detect           : FW.Url().match(/\/topic\//),
choices          : {
  titles  :  FW.Xpath('//div[@class="content"]/h2/a').text().trim(),
  urls    :  FW.Xpath('//div[@class="content"]/h2/a').key("href").trim()
}
});
/** BEGIN TEST CASES **/
var testCases = [
