{
	...
	"lastUpdated": "2017-03-16 07:09:00"
}
...
detect           : FW.Xpath('//h2[@content="Kirkus Book Review"]'),
title            : FW.Xpath('//meta[@property="og:title"]/@content').text().split(/\|/),
attachments      : [{ url: FW.Url(),
  title:  "Kirkus Reviews snapshot",
  type: "text/html" }],
creators         : FW.Xpath('//div[@class="book-author clearfix"]//a/span').text().cleanAuthor("reviewedAuthor"),
...
hooks : { "scraperDone": function  (item,doc,url) {
	// Process title
	item.title = Zotero.Utilities.capitalizeTitle(item.title.toLowerCase(),true).replace (/^/,'Rev. of ').replace(/\s,/,",");
}}
});

FW.Scraper({
itemType         : 'blogPost',
detect           : FW.Xpath('//body[@data-slot-scope="blog-post"]'),
title            : FW.Xpath('//meta[@property="og:title"]/@content').text().split(/\|/),
attachments      : [{ url: FW.Url(),
  title:  "Kirkus Reviews snapshot",
  type: "text/html" }],
creators         : FW.Xpath('(//div[@class="blog-post-teaser-meta-ctr "])[1]').text().replace(/[\n\s]+/g," ").replace(/.*[Bb]y\s([\w\s]+),?.*\son\s.*/,"$1").cleanAuthor("reviewedAuthor"), // http://regexr.com/3fhho
date             : FW.Xpath('(//div[@class="blog-post-teaser-meta-ctr "])[1]').text().replace(/[\n\s]+/g," ").replace(/.*\son\s(.*)/,"$1"),
...
FW.MultiScraper({
itemType         : 'multiple',
detect           : FW.Xpath('//div[@class="page-intro-header"]'), // nav cats
choices          : {
  titles  :  FW.Xpath('(//div[@class="row book-item-ctr"]//span[@itemprop="name"])|(//div[@class="row blog-post-teaser-row"]//div[@class="blog-post-header"]/a)').text().trim(),
  urls    :  FW.Xpath('(//div[@class="row book-item-ctr"]//span[@itemprop="name"]/ancestor::a)|(//div[@class="row blog-post-teaser-row"]//div[@class="blog-post-header"]/a)').key("href")
}
});

FW.MultiScraper({
itemType         : 'multiple',
detect           : FW.Url().match(/\/search\/\?q=/), // search
choices          : {
  titles  :  FW.Xpath('//div[@class="hidden-xs col-sm-12"]/a[1]').text().trim(),
  urls    :  FW.Xpath('//div[@class="hidden-xs col-sm-12"]/a[1]').key("href")
}
});
