{
	...
	"translatorType": 4,
	"browserSupport": "g",
	"lastUpdated": "2013-03-12 00:24:23"
}
...
function doWeb(doc,url)
{
	if (detectWeb(doc, url) == "multiple") {
		var hits = {};
		var urls = [];
		var resultsrow = ZU.xpath(doc,"//table[@id='searchResult']/tbody/tr");
		var link;
		for (var i in resultsrow) {
			link = ZU.xpathText(resultsrow[i], './/a[@class="viewoption" and contains(text(), "Full Text")]/@onclick');
			if (!link) link = ZU.xpathText(resultsrow[i], './/a[@class="viewoption" and contains(text(), "Abstract")]/@onclick');
			link = link.match(/http:\/\/.+(fulltext|abstract)/)[0];
			hits[link] = ZU.xpathText(resultsrow[i], './td[@class="focus"]/text()').trim();
		}
		...
		var abstract = ZU.xpathText(doc, '//div[@class="tContent"]/*[self::h3 or (self::p and not(self::p[@class="note"]))]')
		//Z.debug(abstract)
		...
		translator.setHandler('itemDone', function(obj, item) {
			for (var i in keywords){
				if (keywords[i].textContent.length>0){
		...
		"items": [
			{
				...
				"date": "August 2012",
				"publicationTitle": "American Journal of Medicine, The",
				"volume": "125",
				"issue": "8",
				"publisher": "Elsevier",
