...
record.prototype.translate = function (item) {
	...
	if ((this.getFieldSubfields("200")[0]) && (!(this.getFieldSubfields("245")[0]))) {
		...
		const creatorFields = ["100", "110", "700", "710", "720"];// "111", "711" are meeting name
		for (let tag of creatorFields) {
			let authorTab = this.getFieldSubfields(tag);
			for (let j in authorTab) {
				const datafield = authorTab[j];
				...
				// Skip "SKIP" fields
				if (datafield['4'] && RELATERM[datafield['4']] && RELATERM[datafield['4']] == "SKIP") {
					continue;
				}
				...
				if (datafield['4'] && RELATERM[datafield['4']]) {
					creatorObject.creatorType = RELATERM[datafield['4']];
				}
