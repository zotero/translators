...
for (let filename of fs.readdirSync(repo).sort()) {
  ...
  cache[filename] = {
    ...
    FWLine: ast.comments.find(comment => comment.type === 'Block' && comment.value.trim === 'FW LINE 59:b820c6d')?.loc.line
  }
  try {
...
function conflict(filename) {
  filename = path.join(repo, path.basename(filename))
...
module.exports = {
  ...
  supportsAutofix: true,
    ...
    if (parsed) {
      // remove ' FW LINE 59:b820c6d '
      messages = messages.filter(m => m.line !== parsed.FWLine)
