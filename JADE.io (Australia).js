...
// "Commissioner of Taxation v PepsiCo Inc [2025] HCA 30 (13 August 2025)"
...
// Mapping from common JADE/LIIs court abbreviations to full names
// (Used for reference when needed; we generally store the AGLC abbrev in item.court)
...
// Attempt to locate a PDF link on the page for attachment
function findPDFLink(doc) {
	const link = doc.querySelector('a[href$=".pdf"], a[href*="download"][href*="pdf"], a[download$=".pdf"]');
	return link ? link.href : null;
}

// Attempt to locate a parallel law report citation like "274 CLR 450"
// Preferred law report series ordering for parallel citations (AGLC-aligned)
// Use this to decide which parallel citation to put in the main reporter fields
const LAW_REPORT_PRIORITY = [
    'CLR', 'ALJR', 'ALR', 'FCR', 'NSWLR', 'VR', 'Qd R', 'SASR', 'WAR',
    'NTLR', 'ACTR', 'FamLR', 'FLR', 'IR', 'A Crim R', 'ACSR', 'ATPR', 'ATC'
];

// Find all visible parallel law report citations in the given document
// Returns a de-duplicated list of { volume, reporter, page } objects
...
// Choose the best reporter from a list of parallel citations
// Primary key: reporter priority; Secondary: highest volume number
function pickBestLawReport(citations) {
    ...
    citations.sort((a, b) => {
        ...
        return parseInt(b.volume, 10) - parseInt(a.volume, 10);
    });
    return citations[0];
}

// Extract a Medium Neutral Citation from general page text/metadata
// Matches patterns like "[2006] NSWCA 231" when not present in the <title>
...
// Locate a JADE summary page for the article (often contains parallel citations)
// Locate a JADE summary/outline page likely to contain parallel citations and metadata
function findSummaryURL(doc, url) {
...
// Try to extract a reliable case name from page content
// Try to extract a reliable case name from page content
// Order: citation meta tags → prominent headings → cleaned <title>
function extractCaseName(doc) {
	...
	if (title) {
    let parsed = parseNeutralCitationFromTitle(title);
		if (parsed && parsed.caseName) return parsed.caseName;
...
function isGenericCaseTitle(name) {
    return /BarNet\s+Jade|Find\s+recent\s+Australian\s+legal\s+decisions/i.test(name || '');
}
...
function doWeb(doc, url) {
	...
	if (type === "case") return scrapeCase(doc, url);
	if (type === "statute") return scrapeStatute(doc, url);
}
...
function scrapeCase(doc, url) {
	...
	// Parse MNC from <title> first; may be updated later if not present
	let parsed = parseNeutralCitationFromTitle(title);
	...
	if (parsed) {
        ...
        if (parsed) {
            ...
            item.caseName = fromPage || title.replace(/^\s*BarNet\s+Jade\s*-\s*/i, '').replace(/\s*-\s*BarNet\s+Jade.*$/i, "");
            caseNameStr = item.caseName;
            item.court = "";
        }
    }
    ...
    function applyParallel(prc, allList) {
        ...
        for (const c of toAppend) {
            ...
            if (parallelSegments.indexOf(seg) === -1) parallelSegments.push(seg);
        }
    }
