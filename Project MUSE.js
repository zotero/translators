{
	...
	"lastUpdated": "2012-09-11 15:07:30"
}
...
function doWeb(doc, url) {
	...
	if (detectWeb(doc, url) == "multiple") {
		...
		var results = ZU.xpath(doc, '//div[@class="article"]/h4/a|//div[@class="result_info"]/h1/a');
		if (results.length) {
		...
		Zotero.selectItems(items, function (items) {
			...
			Zotero.Utilities.processDocuments(urls, scrapeOne, function () {
				Zotero.done();
			}, null);
		});
	} else scrapeOne(doc);
				...
				if (authorNodes.length) {
					...
					for (var i = 0, n = authorNodes.length; i < n; i++) {
						...
						if (authors.length == 1) {
							...
							if (authorsByComma.length > 1 && authorsByComma[0].indexOf(" ") !== -1 && authorsByComma[1].indexOf(" ") !== -1) authors = authorsByComma;
						}
